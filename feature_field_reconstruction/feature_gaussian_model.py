...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self._dino_feature = torch.empty(0)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        dino_feature = torch.zeros((fused_color.shape[0], 32)).float().cuda()
        ...
        self._dino_feature = nn.Parameter(dino_feature.requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._dino_feature], 'lr': training_args.dino_feature_lr, "name": "dino_f"},
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._dino_feature.shape[1]):
            l.append('dino_f_{}'.format(i))
        l.append('opacity')
    ...
    def save_ply(self, path):
        ...
        dino_f = self._dino_feature.detach().cpu.numpy()
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, dino_f, opacities, scale, rotation), axis=1)
    ...
    def load_feature_ply(self, path):
        plydata = PlyData.read(path)
        ...
        dino_feature_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("dino_f_")]
        dino_feature_names = sorted(dino_feature_names, key = lambda x: int(x.split('_')[-1]))
        dino_features = np.zeros((xyz.shape[0], len(dino_feature_names)))
        for idx, attr_name in enumerate(dino_feature_names):
        ...
        self._dino_feature = nn.Parameter(torch.tensor(dino_features, dtype=torch.float, device="cuda").requires_grad_(True))
